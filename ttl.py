...
def ttl2ip(ttl, dest):
    ...
    TTL 2..15 is 14 fib numbers and dest itself
    TTL >15 will raise IndexError, and calling function should see
    that as an oppurtunity to send ICMP ECHO REPLY (TODO: TCP/UDP port unreachable) like normal host would.
...
def main():
    blob = ctypes.create_string_buffer(
        b"".join(struct.pack("HBBI", *e) for e in filter)
    )
    ...
    while True:
        ...
        print(
            "%s %16s -> %16s ttl:%03d proto:%-3d"
            % ("*" if ip.ttl == 2 else " ", ip.saddr, ip.daddr, ip.ttl, ip.protocol)
        )
        ...
        try:
            ...
            if (
                ip.protocol == 1
                and data[ipoptslen + p] == 0x08
                and data[ipoptslen + p + 1] == 0x00
            ):
